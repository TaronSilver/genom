...
using namespace std;
vector<size_t> find_sequence(const string& string_to_find, const vector<string>& genes, int sequence_in_gene);


int main(){
		string chain("SOSOAAAAAAAAAAAAASOSOAAAAAAAAAAASOSOAAAAAAAAAAAA");
		vector<string> sequences;
		sequences.push_back(chain);
		string tofind ("SOSO");
		vector<size_t> found (find_sequence(tofind, sequences,0));
